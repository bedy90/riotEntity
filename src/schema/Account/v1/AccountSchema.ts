...
export const AccountSchema_v1 = z.object({
    ...
    gameName: z.string({ error: (issue) => issue.input === undefined ? 'gameName is required' : 'not a string' })
    ...
    tagLine: z.string({ error: (issue) => issue.input === undefined ? 'tagLine is required' : 'not a string' })
